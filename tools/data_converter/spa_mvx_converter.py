...
spa_mvx_categories = ('pedestrian', 'bicycle', 'car', 'motorcycle')
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in mmcv.track_iter_progress(infos):
        ...
        if remove_outside:
            ...
            projected_pc = np.dot(P[0], np.dot(P[1][:3, :3], points_v[:, :3].T)+ P[1][:3, 3].reshape(3, 1)).T
            projected_pc[:, 0] /= projected_pc[:, 2]
            projected_pc[:, 1] /= projected_pc[:, 2]
            pc_mask = box_np_ops_spa_mvx.get_pts_index_in_3dbox__(projected_pc, image_info['image_shape'])
            points_v_ = points_v[pc_mask[0]]
        ...
        # dims = dims[:, [0, 1, 2]] # hwl ==> dxdydz(lwh)
        # rots_ = -rots+np.pi/2

        gt_boxes_lidar = np.concatenate([loc, dims, rots[..., np.newaxis]],
                                         axis=1)
        #corners_3d = box_np_ops_spa_mvx.box_center_to_corner_3d(gt_boxes_lidar[:, :3], gt_boxes_lidar[:, 3:6], gt_boxes_lidar[:, 6])
        #num_pts_list = box_np_ops_spa_mvx.get_pts_in_3dbox_(points_v, np.expand_dims(corners_3d, axis=0))
        # intrinsic = P['cam_intrinsic'][0]
        # extrinsic = P['cam_intrinsic'][1][:3, :3]
        # translation = P['cam_intrinsic'][1][:3, 3]
