...
def get_spa_image_info(path,
    ...
    def map_func(idx):
        ...
        for _, cam in enumerate(cam_list):
            info = {}
            calib_info = {}
            image_info = {'image_idx':idx.split("*")[2]}
            annotations = None
            image_info['image_path'] = cam
            ...
            if label_info:
                label_path = get_label_path(idx.split("*")[2], Path(path,idx.split("*")[0],idx.split("*")[1]), "label/label_{}".format(_+1), training, relative_path)
                if relative_path:
            ...
            if calib:
                ...
                if extend_matrix:
                    P0 = _extend_matrix_([P0_intrinsic, P0_extrinsic])
                    P1 = _extend_matrix_([P1_intrinsic, P1_extrinsic])
                    P2 = _extend_matrix_([P2_intrinsic, P2_extrinsic])
                    P3 = _extend_matrix_([P3_intrinsic, P3_extrinsic])
                    P4 = _extend_matrix_([P4_intrinsic, P4_extrinsic])
                R0_rect = np.array([
                    float(info) for info in lines[10].split(' ')[1:10]
                ]).reshape([3, 3])
                if extend_matrix:
        ...
        return all_infos[0], all_infos[1], all_infos[2], all_infos[3], all_infos[4],

    all_infos = []
    for _, ids in enumerate(image_ids):
        print("processing {} / {}".format(_, len(image_ids)))
        image_infos_0, image_infos_1, image_infos_2, image_infos_3, image_infos_4 = map_func(ids)
        all_infos.append(image_infos_0)
        all_infos.append(image_infos_1)
        all_infos.append(image_infos_2)
        all_infos.append(image_infos_3)
        all_infos.append(image_infos_4)
    return all_infos
...
def add_difficulty_to_annos(info):
    ...
    min_height = [0.1, 0.1,
                  0.1]  # minimum height for evaluated groundtruth/detections
    max_occlusion = [
        0, 1, 2
